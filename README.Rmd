...
each one).  That is what the hpcc-profiles/slurm/sedna profile does.  Study the `config.yaml`
file in the profile:
```yaml
cluster:
...
snakemake  --profile hpcc-profiles/slurm/sedna
```
While that is running, we can look at our jobs with sinfo wrapped up in `myjobs`
...
See how the rule name and the wildcard names and values go into making the job
name!  The snakemake sedna profile does that.

By the way, all of the slurm logs go into a separate directory with well-formed
names:
```sh
...
```
All of that is specified in the `config.yaml` file in the slurm
profile.


If you wanted snakemake to ensure that didn't send too many jobs off at
once, you could ensure that it would pause itself so that there were no more than
10 jobs running at any one time by doing:
```sh
snakemake  --profile hpcc-profiles/slurm/sedna --jobs 10
```

When you run this, you will see that Snakemake uses the `threads` parameters
for each rule to determine how many cores to request. This happens because of the
`--cpus-per-task={threads}` line in the profile, as seen above.
...
If you need something different for a particular rule, you set that in the
rule definition in the Snakefile with a `resources` block, like in the 
Snakefile in this repo:
```python
