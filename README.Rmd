...
SUMMIT has some issues with its ssh agent so you
probably have to do this to get things from GitHub (assuming that you have
your SSH keys set up in `id_ed25519')
```sh
eval "$(ssh-agent -s)"
...
Then, you have to have a snakemake conda environment.
If you don't have that you can install it with mamba.
...
Once that is done, from `scompile` (and also logged in via a TMUX session
so that things don't die after you logout), do this command from within
